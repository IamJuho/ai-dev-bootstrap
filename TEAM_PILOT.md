...
- 대상: macOS, Linux, 또는 Windows 11 Git Bash/MSYS 계열에서 Codex를 실제로 쓰는 팀원 3-5명
- 기본 host: `codex`
- 기본 phase: `core`
- 확장 phase: browser, QA, visual review가 필요한 경우에만 `full`
- 제외: PowerShell native bootstrap, 완전 무감독 Claude 롤아웃
...
- `bun`
- GitHub 접근 가능한 네트워크
- Windows에서는 Git Bash/MSYS 계열 shell
...
Windows `full`에서 `node`가 없다는 오류:

- Windows Git Bash/MSYS 계열의 `full` phase는 `node`를 필수로 요구한다.
- `node`를 PATH에 추가한 뒤 `./bin/setup-dev-agents --host codex --phase full`을 다시 실행한다.

`unsupported platform` 오류:

- 지원 OS는 macOS, Linux, Windows 11 Git Bash/MSYS 계열이다.
- PowerShell native 실행은 파일럿 범위 밖이다. Windows에서는 Git Bash/MSYS 계열 shell로 다시 실행한다.
...
`refusing to remove non-link directory` 또는 `refusing to replace non-directory` 오류:

- 기존 경로가 예상한 bootstrap 산출물이 아니라는 뜻이다.
- Windows에서는 예상 link가 directory junction일 수 있다.
